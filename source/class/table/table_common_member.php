...
class table_common_member extends discuz_table_archive
{
	...
	public function fetch_uid_by_email($email, $fetch_archive = 0) {
		$uid = 0;
		if($email) {
			$uid = DB::result_first('SELECT uid FROM %t WHERE email=%s', array($this->_table, $email));
			if(isset($this->membersplit) && $fetch_archive && empty($uid)) {
				$uid = C::t($this->_table.'_archive')->fetch_uid_by_email($email, 0);
			}
		}
		return $uid;
	}
